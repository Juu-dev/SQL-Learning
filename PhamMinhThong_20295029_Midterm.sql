...
IF OBJECT_ID ('dbo.Gv_dt_view', 'view') IS NOT NULL
DROP VIEW dbo.Gv_dt_view ;

CREATE VIEW dbo.Gv_dt_view
WITH SCHEMABINDING
AS
SELECT
    dt.TenDT, dt.TheLoai, hd.NamThucHien
FROM
    dbo.Giangvien gv
INNER JOIN dbo.Huongdan hd 
    ON gv.GV# = hd.GV#
INNER JOIN dbo.Detai dt 
    ON dt.DT# = hd.DT#;
...
--xoa index--
drop index IDX_v1 ON dbo.Gv_dt_view;
drop index IDX_v2 ON dbo.Gv_dt_view;

--tao index_1
CREATE UNIQUE CLUSTERED INDEX IDX_v1
ON dbo.Gv_dt_view (HoTen, TenDT, TheLoai, NamThucHien);
--tao index_2
...
alter role Duy_Cuong add member NDTien;
...
alter role Duy_Cuong add member PVCuong;
-------------------------------------------